...
if __name__ == "__main__":
    ...
    p.add_argument("fitfile", help="path to output npz file")
    ...
    results = np.load(args.fitfile)
    if "samples" in results:
        pos = results["samples"]
        allowed = [matboundprior(s) for s in pos]
        print("{}\t{}/{} samples in allowed region".format(args.fitfile, sum(allowed), len(allowed)))
    else:
        w0 = results["mle"]
        if matboundprior(w0):
            print("{}\tOK".format(args.fitfile))
        else:
            print("{}\tout of bounds".format(args.fitfile))
