...
data_repo = os.path.join(os.environ["HOME"], "crcns")
...
stims,durations,spikes_data,spiky_data = utils.load_crcns(cell,
                                                          stim_type,
                                                          nspec,
                                                          t_dsample,
                                                          compress,
...
mat_prior = priors.joint_independent(
                [ nf.priors.uniform(bounds["a1"][0],bounds["a1"][1]),
                  nf.priors.uniform(bounds["a2"][0],bounds["a2"][1]),
                  nf.priors.uniform(bounds["w"][0],bounds["w"][1])])
...
p0 = startpos.uniform_independent(nwalkers,[bounds["a1"][0],bounds["a2"][0],bounds["w"][0]],[bounds["a1"][1],bounds["a2"][1],bounds["w"][1]])
#p0 = startpos.normal_independent(nwalkers,[5,0,0,1],[0.1]*4)
#p0 = startpos.normal_independent(nwalkers,matparam,[0.1]*4)

# run emcee
mat_smplr = nf.sampler(mat_model,mat_prior,cost,nwalkers,zip(assim_Iapp,assim_spiky),pool=pool)
...
# run emcee
print("\n")
dstrf_smplr = nf.sampler(model,dstrf_prior,spike_distance,nwalkers,zip(assim_stims,assim_spiky),pool=pool)
